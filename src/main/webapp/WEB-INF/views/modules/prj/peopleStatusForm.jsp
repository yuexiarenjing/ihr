...
<ul class="nav nav-tabs">
	...
	<li><a href="${ctx}/prj/people/status?id=${people.id}">状态列表</a></li>
	<li class="active"><a href="${ctx}/prj/people/status/form?id=${people.id}">状态添加</a></li>
</ul><br/>
<form id="inputForm" action="${ctx}/prj/people/status/save" method="post" class="form-horizontal">
	...
	<input type="hidden" name="pid" value=${people.id}  maxlength="50" />

	<div class="control-group">
		<label class="control-label">姓名:</label>
		<div class="controls">
			<%--<input name="name" value=${people.name == null ? "" : people.name}  class="required" readonly="true"/> --%>
				<input name="name" value=${people.name}  maxlength="50" readonly="true"/>
		</div>
	</div>
